...
EOD;
		...
		$category_inner_html = EDJ_Lightbox::build_category_column($table_headers, $inner_html_format);
		$value_inner_html = EDJ_Lightbox::build_value_column($table_headers, $row, $inner_html_format, $journal_url_html_format);
	 ...
	 */
	 public static function build_category_column($table_headers, $inner_html_format) {
		...
		for ($i = 1; $i < count($table_headers); $i++) {
			...
			$category = EDJ_Functions::underscore_to_whitespace($table_headers[$i]) . ":";
	...
	public static function build_value_column($table_headers, $row, $inner_html_format, $journal_url_html) {
		...
		for ($i = 1; $i < count($table_headers); $i++) {
			 ...
			 if (strpos($value, 'http') === 0) {
				...
				$url_text = EDJ_Functions::get_url_text($table_headers[$i]); 
